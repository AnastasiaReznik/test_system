...
$('#test').submit(function (e) {
    e.preventDefault();
    const tmp = `<button type="submit" class="btn btn-warning reset">Пройти заново</button>`;
    $(".wrapper").html(tmp);
    var cor = 0;
    var uncor = 0;
    $('input[type="radio"]:checked').each(function (index, val) {
        ...
        if (!correct) {
            ...
            // console.log($(this).siblings(".alert").className); 
            // $(this).siblings("alert").removeAttr('hidden');
            // $(this).siblings("alert");
        } else {
            cor++;
